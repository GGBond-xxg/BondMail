...
从 v1.5.5.1 起，点击 AI 服务进入独立管理页面；点击新增或编辑才打开配置窗口。服务商预设菜单与选择框等宽，长表单可滚动，底部保存按钮保持可见。
...
模型选择会隐藏已知向量、重排序、生图和语音模型，对话候选优先显示。模型 ID 无法识别时保留并标注“待验证”，建议测试后使用；仍支持手动输入。此筛选基于已知命名规则，不代表服务商账户权限或模型可用性验证。
