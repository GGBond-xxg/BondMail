...
邮件标题与正文分别翻译并一起展示，回复、转发及原始邮件保持不变。服务与目标语言并排选择，译文 / 对照 / 原文替代复选框；正文独立滚动，底部操作固定。标题和正文均成功后显示结果，已成功部分缓存供重试复用。

验证：34 项 JVM 测试通过（含标题/正文分离、空字段与部分失败检查）；翻译长内容大字体 UI 测试被 vivo i 管家测试包风险提示遮挡，未通过界面验证。

### v1.5.3 HTML 翻译与详情操作栏

邮件在原 HTML 中逐文字节点翻译，保留表格、图片、样式和链接地址。语言与服务密钥选择位于顶部，翻译和原文悬浮按钮位于底部操作栏上方；底部隐藏时使用与首页及联系人相同的共用位移动画。已增加 iFAST 页脚对比度修正及 Aleta Adventure 通用银行图标。
...
关于页更多菜单新增赞助页面，七条公开收款地址支持完整显示、选择与复制，沿用现有返回动画和双界面样式。README 增加上手、赞助及 GPT 协作说明。翻译密钥设置跟随邮件当前服务，取消不改变服务，保存后应用所保存的选择。此版本尚未实现 AI，后续 v1.5.4 实现见下。

### v1.5.4 AI 邮件助手与赞助二维码

自填兼容 OpenAI / Gemini API 配置，按服务加密密钥，邮件更多菜单提供摘要、重点/待办、回复草稿与当前邮件问答。草稿需主动带入编辑页并审核发送，助手关闭后清空对话。输入长度、历史长度、超时、取消与原始错误信息隔离均已实现。七张赞助二维码由地址目录生成并使用独立解码器核对；不增加应用运行时依赖。详见 [AI.md](AI.md)。

验证：46 项 JVM 测试、4 项 Android 离线接口契约测试、1 项 AI 草稿确认交互测试通过；Debug、Performance 构建及 Performance Lint Vital 通过。真机检查了深色配置页、两种服务切换和模拟邮件助手界面；赞助弹层截图也成功解码为对应 SOL 地址。没有调用真实付费 AI 接口，线上权限与额度仍需用户有效密钥验证。

### v1.5.5 服务商预设、模型与主题二维码

加入 DeepSeek、Kimi、Xiaomi MiMo、OpenAI、Gemini 预设与多套自定义配置；支持模型列表获取、搜索选择和手填模型 ID。旧版密钥迁移不改变当前服务；删除当前配置也不会自动启用另一家服务。自定义兼容接口可选择完整 URL 及认证头，MiMo 使用 api-key。

赞助复制与二维码按钮左右并排；二维码按主题映射前景与背景，保留静区和模块形状，深色模式提供标准黑白配色回退。

验证：50 项 JVM 测试、7 项 Android 离线接口/加密迁移测试通过。七张码在浅色与深色配色下均通过独立解码；真实手机截图的深色码及标准回退码均解码为同一正确地址。人工核对了服务商预设、MiMo 模型选择与回填、赞助按钮同排。3 项自动界面测试被 vivo i 管家对测试包的风险提示遮挡，未通过；测试包已按系统提示移除，未绕过安全控制。未调用真实 AI 密钥，线上模型权限及额度仍需自行测试。

### v1.5.5.1 AI 设置页面与弹窗层级

设置中的 AI 服务进入独立导航页面，新增/编辑在管理列表之上显示圆角弹窗；关闭保留列表，返回键回到设置。预设、协议及认证菜单按触发框实际宽度展开；长表单滚动，保存栏固定。独立页面背景覆盖系统导航栏，避免透出上一页内容。

验证：50 项 JVM 测试通过，Performance 构建及 Lint Vital 通过。真机人工检查独立页面、新增弹窗、等宽预设列表、MiMo 回填、表单滚动、固定保存栏与两级返回。未调用线上 AI 接口，未安装或运行此前被系统拦截的自动 UI 测试包。
