...
- 最新版本：[BondMail v1.5.6.2](https://github.com/GGBond-xxg/BondMail/releases/tag/v1.5.6.2)
- 安装包：[BondMail-v1.5.6.2.apk](https://github.com/GGBond-xxg/BondMail/releases/download/v1.5.6.2/BondMail-v1.5.6.2.apk)
...
AI 摘要、解释和回复草稿由你选择的第三方模型生成，可能出现事实错误、遗漏、误解或不当表达，不代表 BondMail 开发者的观点或承诺。

请在采用或发送前核对原邮件、收件人、事实、金额、日期及措辞。应用不会自动发送 AI 回复，最终是否采用、修改或发送由你决定。

在适用法律允许的范围内，开发者不保证 AI 输出的准确性、完整性或适用性，对因依赖、采用或发送 AI 输出而产生的损失不承担责任。本声明不排除或限制依法不得免除的责任，也不影响用户依法享有的权利。

使用 AI 会将邮件标题、正文文字及你的输入发送给你配置的服务商；图片和附件不参与。服务商的数据处理、可用性和费用适用其自身条款。请确认你有权向该服务提供相关内容。
