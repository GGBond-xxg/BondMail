...
- 最新版本：[BondMail v1.5.7.4](https://github.com/GGBond-xxg/BondMail/releases/tag/v1.5.7.4)
- 安装包：[BondMail-v1.5.7.4.apk](https://github.com/GGBond-xxg/BondMail/releases/download/v1.5.7.4/BondMail-v1.5.7.4.apk)
...
BondMail 是邮件管理及辅助工具，不提供、鼓励或授权任何违法用途。请遵守适用法律法规，确保有权访问邮箱、处理邮件及向第三方提供相关内容，不得用于诈骗、骚扰、垃圾邮件、侵犯隐私、侵权或其他违法活动。

用户应对自己配置的账号、接口、技能及收发、转发、上传、发布的内容和操作负责。用户违法、违规或侵权使用所产生的责任，依法由相应责任主体承担；在适用法律允许的范围内，开发者不对用户自行实施的上述行为及其后果承担责任。使用本应用不代表开发者认可或参与用户的行为。

应用按现状提供。在适用法律允许的范围内，开发者不保证功能始终可用、无错误或适合特定目的，对使用或无法使用应用造成的损失不承担责任。重要邮件和配置请自行备份。

AI 摘要、解释和回复草稿及机器翻译可能存在错误、遗漏或不当表达，不代表开发者观点或承诺。采用或发送前请核对原邮件、收件人、事实、金额、日期与措辞；应用不会自动发送 AI 回复，是否采用、修改或发送由你决定。

使用 AI、翻译或其他第三方服务时，相关内容会按功能发送给你配置的服务商。AI 请求包含标题、正文文字及你的输入，不包含图片和附件。服务商的数据处理、费用与可用性适用其自身条款，请确认你有权提供相关内容并妥善保管密钥。

本条款不排除或限制依法不得免除的责任，包括适用法律规定的故意、重大过失等责任，也不影响用户依法享有的权利。责任认定以适用法律和具体事实为准，不能仅凭本条款免除法定责任。
